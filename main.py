...
class State:
    ...
    def update(self, moves: list[Direction]):
        ...
        for actor in self.actors:
            if moves[i] == Direction.PICKUP:
                ...
                space = self.env.at(Position(pickX, pickY))
                blocksLeft = space.num_blocks - 1
                if blocksLeft <= 0:
                  self.env.set(Position(pickX, pickY), Space(SpaceType.EMPTY))
                else:  
                  self.env.set(Position(pickX, pickY), Space(SpaceType.PICK_UP, num_blocks= blocksLeft))
    ...
    def __init__(self, num_states, num_actions):
        ...
        for i in range(5):
            for j in range(5):
                ...
                self.states_index.append(pos)

    def get_q(self, state, action):
        state_index = state.x * 5 + state.y
        action_index = self.actions_index.index(action)
        return self.q_table[state_index, action_index]

    def set_q(self, state, action, value):
        state_index = state.x * 5 + state.y
        action_index = self.actions_index.index(action)
        self.q_table[state_index, action_index] = value

    def print_q_table(self):
        data = []
        for state_index, state_row in enumerate(self.q_table):
            state = Position(state_index // 5, state_index % 5) 
            for action_index, q_value in enumerate(state_row):
                action = self.actions_index[action_index]
                data.append([state, action, q_value])
...
class Policy:
    ...
    def PRANDOM(self, current_state: State, env: Environment, qTable, gamma, alpha) -> list[Direction]:
        operator = []
        i = 0
    ...
    def PGREEDY(self, current_state: State, env: Environment, qTable, gamma, alpha) -> list[Direction]:
        ...
        for actor in current_state.actor_pos:
            ...
            checkSpace = env.at(Position(checkX, checkY))
            #checks if actor can pick up, if so actor picks up box and environment is updated
            if self.checkPickUp(current_state, checkSpace, actor, i) == True:
                operator.append(Direction.PICKUP)
            #checks if actor can drop off, if so actor drops off box and environment is updated
            elif self.checkDropOff(current_state, checkSpace, actor, i) == True:
                operator.append(Direction.DROPOFF)
            else:
    ...
    def valid_actions(self, current_state: State, position, actor: Actor) -> list[Direction]:
        ...
        if invalid != 0 and self.check_position(current_state, (position.x, position.y+1), actor) == True: # NORTH
            actions.append(Direction.NORTH)
        
        if invalid != 1 and self.check_position(current_state, (position.x, position.y-1), actor) == True: # SOUTH
            actions.append(Direction.SOUTH)
        
        if invalid != 2 and self.check_position(current_state, (position.x+1, position.y), actor) == True: # EAST
            actions.append(Direction.EAST)

        if invalid != 3 and self.check_position(current_state, (position.x-1, position.y), actor) == True: # WEST
...
class Run:
    ...
    def explore(self, gamma, alpha) -> QTable:
        ...
        for i in range(500):
            ...
            actions = policy.PRANDOM(current_state, current_state.env, self.table, gamma, alpha)
...
def main():
    ...
    train = exper.explore(gamma = 0.5, alpha = 0.3)
